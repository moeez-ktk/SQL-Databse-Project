...
function Login() {
    ...
    const updateAccepted = async () => {
        ...
        const Data = await fetch('http://localhost:5000/updateAccepted', {
            method: 'POST',
            headers: {
    ...
    const createworker = async (url, uname, pass, firstname, lastname, dob, gender, phone, cnic, email, street, house, city, occup, exp, price, desc) => {
        ...
        const Data = await fetch(url, {
            ...
            .then(Data => {
                console.log(Data[0])
                if (Data[0].status == 'success') {
                    console.log('inside if data.status == success')
                    setSuccessAlert(true)
                    setTimeout(() => {
                        setSuccessAlert(false);
                    }, 3000)
    ...
    const handleSubmit = async (event) => {
        ...
        if (!SignIn) {
            ...
            if (isWorker) {
                ...
                // console.log(occup.value, exp.value)
            ...
            // Compare user info
            if (userData) {
                console.log('inside if user data')
                // console.log(userData.Password,pass.value)
