...
const updateAccepted = async () => {
    try {
        ...
        let customers = await pool.request().query(`update Booking set Status = 'Completed' where Status = 'Accepted' and BookDate< GETDATE() and BookTime< CONVERT(TIME,CURRENT_TIMESTAMP) `)
...
const getWorkerBookings = async (id, date) => {
    try {
        ...
        console.log('BOOK DATE FROM SERVE',worker);
        return worker;
    }
    catch (error) {
        console.log(error);
